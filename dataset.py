...
class SceneTextDataset(Dataset):
    def __init__(
        ...
        color_jitter=True,
        normalize=True,
        map_scale=0.5,  # map_scale 추가
    ):
        if num == 0:
            pkl_dir = osp.join(root_dir, "pickles/{}.pickle".format(split))
        else:
            pkl_dir = osp.join(root_dir, "pickles/{}.pickle".format(split + str(num)))

        print(f"Loading pickle file from: {pkl_dir}")
        ...
        self.to_tensor = True
        self.map_scale = map_scale  # map_scale 저장
    ...
    def __getitem__(self, idx):
        ...
        mask_size = (int(image.shape[1] * self.map_scale), int(image.shape[0] * self.map_scale))  # width, height 순서로 변경
